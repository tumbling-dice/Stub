public final class LazyLinq<T> implements Iterable<T> {
	...
	public LazyLinq<T> static linq(Collection<T> items) {
		return new LazyLinq<T>(items);
	}
	
	public LazyLinq<T> static linq(T[] array) {
	...
	public LazyLinq<T> where(final Predicate<T> p) {
		...
		_evaluator.add(new Func1<Collection<T>, Collection<T>>() {
			@Override
			public Collection<T> call(Collection<T> items, Paradicate<T> p) {
	...
	public LazyLinq<T> skipWhile(final Predicate p) {
	...
	public List<T> toList() {
		...
		if(items instanceof List<T>) return (List<T>) items;
	...
	public Iterator<T> iterator() {
		return _evaluator.eval().iterator();
	}
